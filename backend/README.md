...
- `GET /verify` - Check Telegram bot status
- `POST /webhook` - Receive Telegram updates (set via Telegram API)
- `POST /link-with-code` - Link Telegram using code from /start in bot
- `GET /check-link/{telegram_id}` - Check if Telegram is linked
